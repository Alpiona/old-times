...
void lerSvg(char* argv){
  ...
  XMLElement* leitor;
  string arqEntrada = ".";
  arqEntrada+= argv+caminho+nome+"."+tipo;
...
void idle(){
  ...
  if (keyFlags['w'] == 1){
    ...
    for (Objeto* obj : objetos){
      ...
      if (obj->getTipo() == LIMITE_EXTERNO_ARENA &&
      obj->getRaio() - raioPlayer < sqrt(pow(obj->getCx()-centroXPlayer,2)+pow(obj->getCy()-centroYPlayer-0.1,2))){
        moveFlag[0] = 0;
      }

      else if (obj->getTipo() != LIMITE_EXTERNO_ARENA && obj->getTipo() != OBSTACULO_PULAVEL &&
      raioPlayer+obj->getRaio() > sqrt(pow(obj->getCx()-centroXPlayer,2)+pow(obj->getCy()-centroYPlayer-0.1,2))){
        moveFlag[0] = 0;
      }

      else if (obj->getTipo() == OBSTACULO_PULAVEL  && playerNoAlto == 0 && subiu == 0 &&
      raioPlayer+obj->getRaio() > sqrt(pow(obj->getCx()-centroXPlayer,2)+pow(obj->getCy()-centroYPlayer-0.1,2))){
        moveFlag[0] = 0;
      }
  ...
  if (keyFlags['a'] == 1){
    ...
    for (Objeto* obj : objetos){
      ...
      if (obj->getTipo() == LIMITE_EXTERNO_ARENA &&
      obj->getRaio() - raioPlayer < sqrt(pow(obj->getCx()-centroXPlayer+0.1,2)+pow(obj->getCy()-centroYPlayer,2))){
        moveFlag[1] = 0;
      }

      else if (obj->getTipo() != LIMITE_EXTERNO_ARENA && obj->getTipo() != OBSTACULO_PULAVEL &&
      raioPlayer+obj->getRaio() > sqrt(pow(obj->getCx()-centroXPlayer+0.1,2)+pow(obj->getCy()-centroYPlayer,2))){
        moveFlag[1] = 0;
      }
